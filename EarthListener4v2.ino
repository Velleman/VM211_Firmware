...
#include "src\Tuyav\src\Tuyav.h"                          // Tuya library
#include "src\Tuyav\src\global.h"                         // Tuya library global functions
...
int loggingInterval = 10;   //interval to log the values to the SD card in seconds. Default: 10, min: 1, maximum: 3600 (1 hour)
...
boolean RTCpresent = false;    //boolean to check if a RTC is present (will be updated in boot loop)
int shownMinute;              //var to store the last shown minute on screen
...
/* --- Tuya part --- */
Tuyav tuyav(&Serial3);    //pin 14 & 15 for RX,TX connections to Tuya IOT interface
unsigned long previousTime = 0; //Initialize Time for updating Arbitrary Values
String AV1msg; String AV2msg; String AV3msg; String AV4msg; String AV5msg; String AV6msg; String AV7msg; String AV8msg; String AV9msg; 
...
void loop(void)
{
  ...
  if(runSeconds != lastSecond)
  {
        ...
        /****** update tuya vars ****/
        AV1msg = "EarthListener";
        AV2msg = "eCO2: ";  AV2msg += CO2;  AV2msg += " ppm";
        AV3msg = "TVOC: ";  AV3msg += TVOC; AV3msg += " ppb";
        AV4msg = "Air quality: ";
          if (CO2 < 600){AV4msg += "Excellent Air";}
          else if ((CO2 >= 600) && (CO2 < 800)){AV4msg += "  Good Air   ";}
          else if ((CO2 >= 800) && (CO2 < 1000)){AV4msg += "  Fair Air   ";}
          else if ((CO2 >= 1000) && (CO2 < 1500)){AV4msg += "Mediocre Air ";}
          else if (CO2 >= 1500){AV4msg += "   Bad Air   ";}
          else {AV4msg = +"  missing...  ";}
        AV5msg = "Temp: ";  AV5msg += TEMP_BME280,1; AV5msg +=" °C";
        AV6msg = "Humi: ";  AV6msg += HUMIDITY_BME280,1; AV6msg +=" %";
        AV7msg = "" + lastErrorLine1;
        AV8msg = "Pres: ";  AV8msg += AMBIENTPRESSURE_BME280_c,1; AV8msg +=" mBar";
        AV9msg = "SWversion: ";    AV9msg = AV9msg + SWversion;
                
        tuyav.setUserValue(AV1, AV1msg);
        tuyav.setUserValue(AV2, AV2msg);
        tuyav.setUserValue(AV3, AV3msg);
        tuyav.setUserValue(AV4, AV4msg);
        tuyav.setUserValue(AV5, AV5msg);
        tuyav.setUserValue(AV6, AV6msg);
        tuyav.setUserValue(AV7, AV7msg);
        tuyav.setUserValue(AV8, AV8msg);
        tuyav.setUserValue(AV9, AV9msg);
        ...
        if(RTCpresent)
        {
          ...
          else
          {
           ...
           //print day & date
           tft.setCursor(20, 25);
           tft.print(dayAsString(timeNow.day)); tft.print("  "); 
           tft.setCursor(20, 40);
           if(MetricON)
           {
